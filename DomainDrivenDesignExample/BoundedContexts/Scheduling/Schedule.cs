...
public class Schedule : AggregateRoot<Guid>
{
    ...
    {
        Guard.Against.Default(movieId);
        Guard.Against.Default(hallId);
        ShowTime = Guard.Against.Null(showTime);

        TicketPrice = Guard.Against.Null(price);
        MovieId = movieId;
        HallId = hallId;
    ...
    {
        ShowTime = Guard.Against.Null(newShowTime);
    }
